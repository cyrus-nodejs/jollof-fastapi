...
app = FastAPI(
    ...
    description="FastAPI service for text embeddings and cosine similarity using FastEmbed (low RAM).",
    version="1.0.0",
)

# ───────────────────────────────
# Load lightweight embedding model (works on Render Free Tier!)
# ───────────────────────────────
...
async def generate_embedding(req: TextRequest) -> Dict[str, List[float]]:
    ...
    try:
        ...
        embedding = list(embedder.embed([req.text]))[0]
